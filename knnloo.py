...
def accuracy_metric(expected, predicted):
    ...
    accuracy = (1.0 * (truenegative + truepositive)) / (
    truenegative + truepositive + falsenegative + falsepositive) * 100
    print "Accuracy :", accuracy, "%"
    MCC = (1.0 * ((truepositive * truenegative) - (falsepositive * falsenegative))) / (math.sqrt(
...
def svm(groupsX, groupsY):
    ...
    for i in range(len(groupsX[0])):
        ...
        train.extend(zip(*Train))
        Test = [X_test, Y_test]
        test.extend(zip(*Test))
        predict, expect = algorithm(train, test, 5)
